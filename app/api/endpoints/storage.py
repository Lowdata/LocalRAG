...
from app.schemas.api import IngestResponse, DocumentsListResponse, DocumentInfo, DeleteResponse
...
def get_ingestion_service(store: VectorStore = Depends(get_vector_store)) -> IngestionService:
...
async def list_documents(store: VectorStore = Depends(get_vector_store)):
    try:
        ...
        records = store.table.search().select(["document_path", "metadata_json"]).to_arrow().to_pylist()
...
async def delete_document(document_path: str, store: VectorStore = Depends(get_vector_store)):
    try:
