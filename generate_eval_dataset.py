...
async def main():
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        for f in doc_files:
            ...
            with open(f, "rb") as fd:
                await client.post("http://localhost:8000/api/v1/ingest", files={"file": (f, fd, "text/markdown")})
    
    # 3. Read chunks from DB
    db = lancedb.connect("data/lancedb")
    table = db.open_table("chunks")
    chunks = table.search([0.0]*384).limit(100).to_list() # getting all since limit 100 > num chunks
    ...
    count = 0
    for chunk in chunks:
        if len(chunk["text"].split()) < 20: continue # skip small chunks
        if count >= 20: break
