...
async def run_evaluation(url: str, questions: List[str]) -> Dict[str, float]:
        ...
        for q in questions:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"Latency: {latency:.2f}s | Sources retrieved: {len(sources)}"
                    )
    ...
    return {
        ...
        "success_rate": (
            successful_queries / len(questions) if len(questions) > 0 else 0
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate RAG Pipeline Latency and Responses"
    )
