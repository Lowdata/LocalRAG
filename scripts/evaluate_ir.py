...
DATASET = [
    {
        ...
        "relevant_chunk_ids": ["eval_corpus.md_page1_chunk0", "eval_corpus.md_page1_chunk1"]
    },
    {
        "question": "Which embedding model is utilized and what is its dimensionality?",
        "relevant_chunk_ids": ["eval_corpus.md_page1_chunk1", "eval_corpus.md_page1_chunk2"]
    },
    {
        "question": "What is the primary bottleneck in the system?",
        "relevant_chunk_ids": ["eval_corpus.md_page1_chunk2", "eval_corpus.md_page1_chunk3"]
    },
    {
...
async def run_evaluation():
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        with open(EVAL_FILE, "rb") as f:
            ...
            try:
                await client.post(INGEST_URL, files={"file": (EVAL_FILE, f, "text/markdown")})
    ...
    latencies, total_mrr, total_recall, total_precision, total_ndcg = [], 0.0, 0.0, 0.0, 0.0
        ...
        for idx, item in enumerate(DATASET):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"  Latency: {latency:.2f}s | MRR: {mrr:.2f} | Recall@3: {recall:.2f} | nDCG@3: {ndcg:.2f} | Prec@3: {precision:.2f}")
    ...
    if successful_queries > 0:
        ...
        print(f"Retrieval - Context Precision:  {total_precision/successful_queries:.2%}")
        print(f"Retrieval - MRR:                {total_mrr/successful_queries:.2f}")
        print(f"Retrieval - nDCG@3:             {total_ndcg/successful_queries:.2f}")
        print(f"Answer    - Faithfulness:       {total_faithfulness/successful_queries:.2%}")
        print(f"Answer    - Relevance:          {total_relevance/successful_queries:.2%}")
